...
import body_parser from 'body-parser';//imports body-parser
...
import database from "./database.mjs";//Accesses our database connection file
const { initDB, addUser, getUser, loginUser, addList, deleteUser, deleteID, getLists, addItem, getItems, getRandom, updateList, deleteList, updateItem, deleteItem, updateUser } = database;
import apiRoutes from "./api-routes.mjs";//Accesses API router file that we created
const port = process.env.PORT || 8080;//Sets up server port
import mongodb from 'mongodb';//imports mongodb to integrate MongoDB Atlas database
const { MongoClient } = mongodb;
import jwt from 'jsonwebtoken';//imports JWT authentication to our API
...
function verifyToken(req, res, next){ //verifies JWT token in authorization header and verifies if the token is authentic
    ...
    if(typeof bearer != undefined){
        ...
        //let token = bearerToken[1];
        req.token = bearer;
        verify(req.token, '', (err,authorizedData)=>{
            if(err){
                res.status(401).json({
                    'message':'Access Denied'
                })
...
app.post("/userdelete", verifyToken, async function(req,res){ //deletes a user (same as delete /account/:accountid below) but requires no parameters
    ...
    try{
        let deleteStatus = await deleteUser(username);
        if(deleteStatus == 0){
